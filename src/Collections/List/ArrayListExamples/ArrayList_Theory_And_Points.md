...
* **Insert/Delete (Middle of list): `O(n)`** - If you insert or remove an element in the middle (or beginning), all subsequent elements must be shifted one position to the right (for insertion) or left (for deletion). This is slow.6
